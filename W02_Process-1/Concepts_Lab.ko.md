...
1. **파이프 EOF:** fork+pipe 프로그램에서 부모가 `read(fd[0])`를 호출하기 전에 `fd[1]`을 닫지 않았다. 자식은 데이터를 쓰고 `fd[1]`을 닫았다. 부모의 `read()`가 반환될 수 있는가? 그 이유는?
   ...
   > **정답:** 아니다 — 부모의 `read()`는 **영원히 블록**된다. 파이프의 `read()`는 **모든** 쓰기 측 파일 디스크립터가 닫혀야 EOF(0)를 반환한다. 부모가 여전히 자신의 `fd[1]` 복사본을 쥐고 있으므로 커널은 파이프가 아직 쓰기 가능하다고 본다. 부모도 자신의 `fd[1]`을 close해야 한다.

2. **dup2 동작:** `dup2(fd, STDOUT_FILENO)` 이후 프로세스가 `printf()`를 호출하면 어떤 일이 발생하는가? 이후에 `close(fd)`를 하는 것이 왜 중요한가?

   > **정답:** `STDOUT_FILENO`(=1)가 이제 `fd`와 같은 open file 엔트리를 가리키므로, `printf()` 출력이 터미널이 아닌 해당 파일(혹은 파이프)로 간다. 이후 `close(fd)`를 하는 이유는 중복된 fd를 해제하기 위함이다 — 그렇지 않으면 fd 테이블 슬롯이 낭비되고, 근본 파일이 필요 이상으로 오래 열린 채로 남아 flush/정리가 지연된다.

3. **fork + exec 패턴:** `exec()`가 성공하면 `perror("exec failed")` 줄이 실행될 수 있는가? 해당 에러 메시지를 보게 된다면 무엇을 의미하는가?

   > **정답:** **아니다** — exec가 성공하면 프로세스 이미지 전체가 대체되므로 호출 이후 코드로 제어가 돌아오지 않는다. 에러 메시지를 보면 `exec()` 자체가 실패한 것이다(잘못된 경로, 권한 부족, 아키텍처 불일치 등).

4. **wait(NULL) vs wait(&status):** 둘의 차이를 설명하라. 각각 어떤 경우에 사용하는가?

   > **정답:** `wait(NULL)`은 자식 종료를 기다리지만 종료 상태를 **버린다**. `wait(&status)`는 `status`에 종료 정보를 채우고, `WIFEXITED(status)` / `WEXITSTATUS(status)`로 검사한다. 부모가 자식의 성공·실패를 알아야 하면 `&status`를, 단지 좀비 방지용 수거라면 `NULL`을 쓴다.

5. **파이프라인 구현:** `ls | wc -l` 구현에서 부모가 파이프에 읽거나 쓰지 않는데도 `fd[0]`과 `fd[1]`을 **모두** 닫아야 하는 이유는 무엇인가?

   > **정답:** 부모는 파이프를 사용하지 않지만, 어느 한쪽이라도 열려 있으면 커널은 읽기 측에 EOF를 신호하지 않는다. 특히 부모가 `fd[1]`을 열어두면 `wc`는 영원히 EOF를 못 받아 블록된다. 부모가 양쪽을 모두 닫아야 "쓰기 측(`ls`)이 끝나면 읽기 측(`wc`)이 EOF를 관찰하고 종료"되는 조건이 성립한다.
