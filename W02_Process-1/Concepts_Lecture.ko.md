...
> 3. 생명주기 다이어그램을 통해 프로세스 상태 전이를 추적할 수 있다
> 4. 좀비 프로세스(Zombie Process)와 고아 프로세스(Orphan Process)를 식별하고 설명할 수 있다
...
> **용어 정의:** 다중화(Multiplexing)란 CPU를 여러 프로세스 사이에서 빠르게 전환하며 공유함으로써, 동시에 실행되는 것처럼 보이게 하는 기법이다.
...
**스택의 성장:**
- 함수가 호출될 때마다 **활성화 레코드(activation record)**(함수 매개변수, 지역 변수, 복귀 주소)가 스택에 push된다. 활성화 레코드(Activation Record) — **스택 프레임(Stack Frame)** 이라고도 한다 — 는 함수가 호출될 때마다 스택에 푸시되는 메모리 블록으로, 함수의 지역 변수, 매개변수, 함수 종료 시 돌아갈 주소를 저장한다.
...
`dec`과 `hex` 열은 모든 섹션의 크기를 합산한 값을 각각 10진수와 16진수로 표시한다. 컴파일 후 `size ./프로그램명`을 실행하면 직접 확인할 수 있다.
...
> **용어 정의:** **타임 슬라이스(Time Slice)** 또는 퀀텀(Quantum)이란, 운영체제가 각 프로세스에게 선점(preempt)하기 전까지 할당하는 고정된 CPU 시간을 말한다.
...
| **CPU 레지스터** | 누산기, 인덱스 레지스터, 스택 포인터, 범용 레지스터, 조건 코드(Condition Codes: 산술 연산 후 CPU가 설정하는 플래그, 예: 제로 플래그, 캐리 플래그 — 마지막 연산 결과의 속성을 나타냄) 등 |
...
> **왜 이 필드들인가?** 이 필드들 중 **프로그램 카운터(Program Counter)** 와 **프로세스 상태(Process State)** 가 가장 중요하다 — 실행이 어디서 재개되는지, 그리고 프로세스가 실행 가능한 상태인지를 결정하기 때문이다.
...
> **참고:** `->` 연산자는 포인터를 통해 필드에 접근한다 — `current->state`는 '`current`가 가리키는 구조체의 `state` 필드를 참조하라'는 뜻이다.
...
> **예시 — 다중 스레드 워드프로세서:** 스레드 1은 사용자 입력을 관리하고, 스레드 2는 맞춤법 검사기를 실행한다 — 둘이 동시에 실행될 수 있다! 이것은 위에서 설명한 단일 스레드의 한계를 극복한다.

**같은 프로세스의 스레드들이 공유하는 것:** 텍스트 영역, 데이터 영역, 힙, 열린 파일, 시그널 등

**스레드마다 독립적인 것:** 프로그램 카운터(PC), 레지스터 세트, 스택

> 스레드를 지원하는 시스템에서는 PCB가 각 스레드의 정보를 포함하도록 확장된다. 스레드를 지원하기 위해 시스템 전반에 걸친 변경이 필요하다.
...
> **적절한 혼합이 중요한 이유:** I/O 바운드 프로세스만 존재하면 레디 큐가 비어 CPU가 유휴 상태(idle)가 된다. CPU 바운드 프로세스만 존재하면 대기 큐가 비어 I/O 장치가 유휴 상태가 된다.
...
> I/O 바운드 프로세스는 I/O를 기다리기 전까지 수 밀리초(milliseconds)만 실행된다. CPU 바운드 프로세스는 더 오래 실행되지만, 스케줄러가 강제로 CPU를 회수한다.

**스와핑(Swapping):**
- 프로세스를 메모리에서 디스크로 이동(**스왑 아웃**)하고 나중에 다시 적재(**스왑 인**)한다.
- 다중 프로그래밍 정도를 줄여 메모리 압박을 완화한다.
- 메모리가 **과다 할당(overcommitted)** 된 경우에만 필요하다 (즉, 모든 실행 중인 프로세스가 요구하는 총 메모리가 사용 가능한 물리 RAM을 초과하는 상태). Ch 9에서 상세히 다룬다.

> **용어 정의:** 스와핑(Swapping)이란 프로세스 전체를 주기억장치(Main Memory)에서 디스크로 내보내고(swap out), 다시 들여오는(swap in) 것으로, 다른 프로세스가 사용할 메모리를 확보하기 위한 기법이다.
...
> **비유:** 시험을 치는 도중 다른 시험지로 전환해야 하는 상황을 상상해 보자 — 현재까지의 답안을 저장하고(레지스터 상태 저장), 현재 시험지를 치우고(PCB 저장), 새 시험지를 꺼낸다(PCB 복원). 시험지를 정리하는 데 쓰는 시간은 낭비(오버헤드)이지만, 이를 통해 여러 시험을 "동시에" 치를 수 있다.
...
- 이후 iOS 버전들은 하드웨어 성능이 향상됨에 따라 더 풍부한 멀티태스킹 지원을 도입했다.
...
> **용어 정의:** `systemd`(또는 이전 시스템의 `init`)는 리눅스 커널이 부팅 시 실행하는 최초의 사용자 공간 프로그램으로, 이후 모든 서비스를 시작하므로 모든 사용자 프로세스는 궁극적으로 PID 1의 자손이다.
...
> 자원 제한(Resource limits)은 과도한 자식 프로세스 생성을 방지한다.
...
3. 자식은 부모의 권한(privileges), 스케줄링 속성(scheduling attributes), 열린 파일(open files) 등을 상속받는다.

> **왜 전체 주소 공간을 복사하는가?** 이 설계는 COW(Copy-On-Write) 최적화를 가능하게 한다 — 처음에는 페이지를 공유하다가 쓰기가 발생할 때만 복사하므로, fork() 직후 exec()를 호출하는 경우 fork()의 비용이 거의 0에 가깝다.
...
> **Windows 구조체:** `STARTUPINFO`는 새 프로세스의 창 크기, 외형, 표준 I/O 핸들을 지정한다. `PROCESS_INFORMATION`은 생성된 프로세스와 스레드의 핸들 및 식별자를 포함한다.
...
```c
exit(1);  // 자식이 부모에게 상태를 반환
```

```c
pid = wait(&status);  // 부모가 자식의 종료 상태를 수집
```

> 프로세스를 강제 종료하려면: Linux에서는 `kill` 명령어 또는 `kill()` 시스템 호출을 사용한다. Windows에서는 `TerminateProcess()` API를 사용한다.

**연쇄 종료(Cascading Termination):** 부모가 종료되면 모든 자식이 종료되어야 하는 시스템. 일반적으로 운영체제에 의해 시작된다.

> **참고:** 리눅스는 기본적으로 연쇄 종료를 구현하지 **않는다** — 고아 자식 프로세스는 대신 init/systemd로 재부모화된다 (3.7절 참조).
...
- **고아 프로세스 정리 절차:** 1. 부모 종료 -> 2. 고아가 init/systemd에 입양됨 -> 3. init/systemd가 주기적으로 wait()를 호출 -> 4. 프로세스 테이블 항목이 해제됨.
- Linux에서는 systemd 이외의 프로세스도 고아 프로세스를 상속받아 관리할 수 있다.
...
> **암기 도우미:** 좀비(Zombie) = 죽었지만 프로세스 테이블에 여전히 남아 있다 (눕지 않는 좀비처럼). 고아(Orphan) = 부모가 사라져서 init/systemd가 입양한다 (고아원에 맡겨진 아이처럼).
...
> **용어 정의:** **시그널(Signal)** 은 OS가 프로세스에 특정 동작을 유발하기 위해 보내는 소프트웨어 알림이다. `SIGCHLD`는 자식 프로세스가 종료될 때 부모에게 자동으로 전송된다.
...
> 프로세스가 여러 역할을 수행하는 경우, 가장 높은 순위가 적용된다.
...
- **p** (path): PATH 환경 변수에서 실행 파일을 검색. `PATH` 환경 변수는 전체 경로 없이 명령어 이름을 입력할 때 OS가 검색하는 디렉토리 목록이다 (예: `/bin`이 PATH에 있으므로 `ls`만 입력해도 실행됨). 콜론(`:`)으로 구분된다.
...
> **참고:** OS는 여러 정보(정상 종료 vs 시그널 종료, 종료 코드, 시그널 번호)를 하나의 `status` 정수의 서로 다른 비트 범위에 인코딩한다. `W*` 매크로는 필요한 특정 정보를 추출하므로 비트를 직접 검사할 필요가 없다.
...
1. **프로세스 상태:** 프로세스가 현재 디스크 I/O 완료를 기다리고 있다. 이 프로세스의 상태는 무엇인가? 어떤 이벤트가 발생하면 Ready 상태로 전이되는가?

   > **정답:** 프로세스는 **Waiting(Blocked)** 상태이다. 디스크가 I/O 완료 인터럽트를 발생시키면 커널의 ISR이 대기 중이던 프로세스를 **Ready**로 표시해 준비 큐에 넣고, 스케줄러가 다음에 선택할 수 있게 된다.

2. **fork() 반환 값:** `pid = fork()` 이후, 부모 프로세스와 자식 프로세스에서 `pid`의 값은 각각 무엇인가? 자식은 자신의 PID를 어떻게 얻을 수 있는가?

   > **정답:** **부모**에서 `pid`는 자식의 PID(양수), **자식**에서 `pid`는 **0**이다. 자식이 자신의 PID를 얻으려면 `getpid()` 시스템 콜을 호출한다.

3. **좀비 vs 고아:** 프로세스 A가 자식 프로세스 B를 생성한 뒤 `wait()`를 호출하지 않고 무한 루프에 진입한다. 프로세스 B가 실행을 마치면 B는 좀비인가, 고아인가? A가 B보다 먼저 종료되면 어떻게 달라지는가?

   > **정답:** B는 **좀비(zombie)** 가 된다 — 실행은 끝났지만 부모가 `wait()`를 부르지 않아 PCB와 종료 상태가 남는다. 반대로 **A가 먼저 종료**되면 B는 `init`(PID 1)에 재부모화되어 **고아(orphan)** 가 되고, init이 B 종료 시 자동으로 `wait()`해 주므로 좀비가 되지 않는다.

4. **문맥 교환 비용:** 문맥 교환이 왜 순수 오버헤드로 간주되는가? 문맥 교환 시간을 줄일 수 있는 하드웨어 기능 두 가지를 말하라.

   > **정답:** 문맥 교환 중 CPU는 레지스터·주소 공간 상태 저장/복원만 할 뿐 사용자 작업을 전혀 수행하지 않으므로 순수 오버헤드이다. 도움이 되는 하드웨어 기능: ① **다중 레지스터 셋 / 레지스터 윈도(register window)**(SPARC) — 레지스터를 메모리에 저장할 필요가 없음, ② **ASID(Address Space Identifier)를 가진 태그드 TLB** — 주소 공간 전환 시 TLB 플러시 불필요.

5. **fork + exec:** 셸이 단일 "생성-후-실행" 시스템 콜 대신 `fork()` + `exec()` + `wait()` 패턴을 사용하는 이유를 설명하라. fork와 exec를 분리하는 것이 어떤 이점을 제공하는가?

   > **정답:** `fork()`와 `exec()` 사이에 자식(부모의 복사본)은 환경을 재설정할 수 있다 — `dup2()`로 파일 디스크립터 재배치, 환경 변수 변경, 작업 디렉터리 변경, 시그널 핸들러 설치 등 — 그 다음에 새 프로그램을 로드한다. 단일 `spawn()` 콜이었다면 부모가 호출 시점에 모든 커스터마이즈 옵션을 미리 지정해야 했을 것이다. fork/exec 분리는 "커스터마이즈 후 로드"라는 단순하고 조합 가능한 인터페이스를 제공한다.
