# 중간고사 Summary Sheet (시험장 지참용) — W01~W07

> **사용법**: 오픈북 시험 중 **빠른 조회용**. 정의·공식·알고리즘 절차·비교표·API 시그니처를 응축했다. 깊은 이해는 `MidtermReview_Total.ko.md` 로 준비하고, 시험장에는 이 파일만 가져가면 모든 문제를 풀 수 있도록 구성했다.
> **읽는 법**: 굵은 글씨 = 핵심 키워드, 표 = 비교·정리, 코드블록 = 시그니처·절차·공식.
...
### 계산 문제 (스케줄링)
- [ ] 간트 차트(Gantt Chart) 그리기
- [ ] 완료 시각·반환 시간(Turnaround Time)·대기 시간(Waiting Time) 표
- [ ] 평균 계산 과정 명시

### 코드 문제
- [ ] 주석으로 의도 설명
- [ ] 에러 처리 (간단히)
- [ ] 자원 해제 (close, free, unlink)

### 깊은 이해 포인트 (보너스)
- [ ] **"왜"** 이렇게 설계됐는지 언급
- [ ] **다른 개념과 연결** (예: TLB ↔ 스레드(Thread) 전환 속도)
- [ ] **한계·예외** 명시
- [ ] **실무 예시** 추가 (Chrome, Linux, PostgreSQL 등)
- [ ] **트레이드오프** 구체적 수치/사례
...
- 먼저 전체 훑기 (3분)
- 확실한 문제부터 (공식·정의)
- 계산 문제: 간트 차트(Gantt Chart) 반드시 그리기
- 서술형: 답안 작성 체크리스트 참고
- 마지막 5분: 검토 (특히 fork 반환값·close 규칙 등)
