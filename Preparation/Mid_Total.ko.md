# 중간고사 총정리 심화본 — W01~W07 완전 학습서

> **이 문서의 목표** — 이 한 권만 읽어도 W01~W07 의 모든 개념을 **처음부터 깊이** 이해할 수 있도록 작성했습니다. 단순 요약이 아니라 **"왜 이 개념이 필요한가 → 어떻게 작동하는가 → 구체적인 예시/코드/계산 → 다른 개념과의 연결"** 순서로 서술했습니다. 교수님이 보너스 점수를 주신다는 "깊은 이해가 엿보이는 답안" 을 쓸 수 있도록 **왜(Why)** 와 **메커니즘(How)** 을 집요하게 설명합니다.
>
> **사용법**
> - 이 문서로 **공부** 하세요. 이해 중심.
> - 시험장에는 `MidtermReview_SummarySheet.ko.md` 를 가져가세요. 조회용.
>
> **주차 범위**
> W01 OS 소개 · W02 프로세스(Process)(1) · W03 프로세스(2)/IPC · W04 스레드(Thread)(1) · W05 스레드(2) · W06 CPU 스케줄링(1) · W07 CPU 스케줄링(2)
...
이 한 권으로 W01~W07 을 처음부터 이해할 수 있도록 **왜(Why)** · **메커니즘(How)** · **예시** · **연결** 을 모두 담았습니다. 공부하시다가 "왜 이 개념이 여기 나오지?" 하는 의문이 들면 **8장 개념 연결 고리** 를 다시 읽으시면 구조가 잡힐 것입니다.

교수님이 보너스 점수를 주신다는 "깊은 이해 답안" 의 핵심은 단순 암기가 아니라 **"이 개념은 왜 존재하고, 다른 어떤 개념과 맞물려 있으며, 어떤 트레이드오프를 갖는가"** 를 체화하는 것입니다. 이 문서의 모든 🔗 연결, 💡 직관, ⚠️ 함정, 🎯 핵심 포인트에 주목해 주세요.
