...
1. **공유 메모리 vs 메시지 전달**: 비디오 스트리밍 애플리케이션에서 디코더 프로세스와 렌더러 프로세스 사이에 4K 프레임(수 MB)을 같은 머신에서 전달해야 한다. 어떤 IPC 모델을 선택하겠는가, 그리고 그 이유는?
   ...
   > **정답:** **공유 메모리**. 4K 프레임은 수 MB이므로 메시지 전달이면 프레임마다 커널 버퍼 복사(사용자 → 커널 → 사용자) 비용이 커진다. 공유 메모리는 두 프로세스가 같은 물리 프레임을 매핑해 **제로 카피**로 접근 가능하므로 고대역폭·저지연 스트리밍에 적합하다. 동기화(누가 생산/소비 중인지)는 세마포·뮤텍스로 별도 처리.

2. **유한 버퍼**: 원형 버퍼 구현에서 왜 `BUFFER_SIZE`개가 아닌 `BUFFER_SIZE - 1`개만 저장 가능한가? 모든 슬롯을 사용하면 어떤 문제가 발생하는가?

   > **정답:** "비어 있음"과 "가득 참"이 모두 `head == tail`로 같아져 구별할 수 없기 때문이다. 한 슬롯을 비워두면 가득 참 = `(head + 1) % BUFFER_SIZE == tail`, 비어 있음 = `head == tail`로 명확히 구분된다. 모두 사용하려면 별도 카운터나 플래그가 필요해 구현이 복잡해진다.

3. **파이프와 EOF**: `fork()` 후에 부모와 자식 모두 `fd[0]`과 `fd[1]`의 복사본을 가진다. 쓰기 측이 `close(fd[READ_END])`를 잊으면 읽기 측이 왜 영원히 블로킹되는지 설명하라.

   > **정답:** EOF는 **모든** 쓰기 측 fd가 닫혔을 때만 신호된다. 부모든 자식이든 누가 하나라도 `fd[WRITE_END]`를 열어 두고 있으면 커널은 파이프가 아직 쓰기 가능하다고 보아 `read()`를 무한 대기시킨다. 쓰기 측 프로세스는 사용하지 않는 모든 fd를 반드시 close해야 한다. *(교재 관용구상 실제 흔한 실수는 읽기 프로세스에서 `fd[WRITE_END]`를 close 안 하는 경우이지만, 원리는 대칭적.)*
...
4. **RPC 의미론**: 은행 애플리케이션이 `transfer(from, to, amount)` RPC를 제공한다. "최대 한 번(at most once)"과 "정확히 한 번(exactly once)" 중 어떤 의미론을 사용해야 하는가? "최소 한 번(at least once)"을 사용하면 어떤 문제가 발생할 수 있는가?
   ...
   > **정답:** **정확히 한 번(exactly once)**. "최소 한 번"은 네트워크 장애 시 재전송으로 같은 이체가 두 번 실행돼 중복 인출/입금이 발생한다. "최대 한 번"은 메시지 손실 시 이체가 아예 실행되지 않을 수 있다. 은행은 중복도 누락도 용납 불가이므로, 서버가 **고유 트랜잭션 ID**로 재시도를 중복 제거하여 정확히 한 번을 구현해야 한다.

5. **직접 통신 vs 간접 통신**: 워커 프로세스가 자주 생성되고 소멸되는 시스템에서, 직접 통신보다 간접 통신(메일박스)이 유리한 점을 하나 서술하라.

   > **정답:** 송신자가 워커의 PID나 주소를 알 필요 없이 이름이 붙은 메일박스로 메시지를 보낼 수 있다. 워커가 추가·제거되어도 송신자 코드는 그대로다. 여러 워커가 같은 메일박스를 구독하면 각자가 다음 메시지를 가져가는 방식으로 자연스러운 부하 분산이 가능하다.
