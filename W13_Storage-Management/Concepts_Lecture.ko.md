...
> **[Computer Architecture]** 이것은 11주차에서 본 **메모리 계층** 의 맨 아래다 — register → cache → 주 메모리 → 디스크, 각 계층이 용량을 지연과 교환. 컴퓨터 구조 과목은 이를 *사이클* 로 정량화: register 접근은 ~1 사이클, DRAM 접근은 수십~수백 사이클이지만, ~12 ms의 단일 HDD 접근은 **수천만 CPU 사이클** 규모. DRAM과 디스크 사이의 그 절벽이 바로 OS가 "이미 메모리에 있는가?"(페이지 폴트 처리, 페이지 캐시)를 핵심 성능 질문으로 다루는 이유이고, 원시 CPU 속도가 아니라 82-IOPS 천장이 그토록 많은 실제 워크로드를 제한하는 이유다.
...
```
  Block A: [Valid][Invalid][Invalid][Valid]   → 유효 페이지를 빼내 복사
...
> **[Computer Architecture]** `CPU → HBA → 버스 → Device Controller → Media` 경로는 컴퓨터 구조 과목의 **I/O 서브시스템** 이다. CPU는 바이트를 하나씩 복사하지 않음; 컨트롤러를 프로그래밍하고 **DMA (Direct Memory Access)** 가 블록을 주 메모리로 바로 스트리밍하게 한 뒤, 완료 시 **인터럽트** 를 올림. HBA와 device controller는 메모리 맵 I/O와 인터럽트 구동 전송을 추론하는 데 쓰는 "공유 버스 위의 I/O 컨트롤러" 모델과 동일 — 저장 장치는 그 버스에 매달린 하나의 장치 부류일 뿐이며, 이는 버스 토폴로지(PCIe 레인 vs 레거시 SATA 링크)가 처리율을 직접 제한하는 이유이기도 하다.
...
> **참고:** 위의 모든 항은 **절대 거리** (head 이동의 크기)다. `(0-65)` 같은 항은 cylinder 0에서 65로 올라가는 이동의 약식 표기이며 `|0-65| = 65` 를 기여할 뿐 음수가 되지 않는다 — head 이동은 결코 음수가 아니다.
...
| C-LOOK (up)     | **322**       | C-SCAN 개선, 디스크 끝 여행 없음                |
...
- **AHCI (Advanced Host Controller Interface).** **SATA** 장치와 통신하기 위한 표준 호스트 컨트롤러 레지스터 인터페이스 — OS 드라이버가 명령을 발행하는 방식, 명령 큐(NCQ) 관리, 핫플러그 처리를 정의. NVMe는 PCIe 부착 SSD를 위한 그 후계자이며, AHCI는 SATA 시대의 대응물로 남아 있음.
...
**복구:** Disk 1이 죽으면 $A_2 = A_1 \oplus A_3 \oplus A_p$. 이것이 작동하는 이유는 **XOR이 자기 자신의 역원** 이기 때문 ($a \oplus b \oplus b = a$): $A_p = A_1 \oplus A_2 \oplus A_3$ 이므로, *생존* 디스크 전부(여기서는 $A_1, A_3, A_p$)를 XOR하면 잃어버린 항을 제외한 모든 항이 상쇄되어 그것을 정확히 재구성한다.
...
> **[Discrete Mathematics]** 단순 XOR로는 *두* 장애를 견딜 수 없다: 미지수가 둘이면 **독립적인** 방정식 두 개가 필요한데, 두 번째 XOR 방정식은 첫 번째에 선형 종속이 되어버린다. 그래서 RAID 6은 Q를 **갈루아 체(Galois field)** $GF(2^8)$ 위에서 계산한다 — 바이트를 $GF(2)$ 위의 다항식으로 보고 기약 다항식으로 나눈 나머지로 다루는 유한체 산술. 각 데이터 블록을 합산 전에 서로 다른 체 상수로 곱하므로 P와 Q가 선형 독립인 두 방정식을 이룬다. 두 잃어버린 블록의 복구는 그러면 **체 위의 2×2 선형 연립방정식 풀이** 일 뿐 — 가우스 소거법의 유한체 대응물이며, 0이 아닌 체 원소는 가역이므로 항상 풀린다.
...
기대 총합: **FCFS 640, SCAN 236, C-SCAN 382, LOOK 208, C-LOOK 322**.
