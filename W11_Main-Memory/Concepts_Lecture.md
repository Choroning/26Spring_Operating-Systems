...
- [2. Address Binding and the MMU](#2-address-binding-and-the-mmu)
  ...
  - [2.7 Segmentation](#27-segmentation)
...
So far the MMU has presented memory as a single linear range. **Segmentation** is an alternative (and historically older) MMU scheme that exposes the address space the way a *programmer* thinks of it: as a collection of variable-sized **segments** — one for the code, one for the global data, one for the stack, one per shared library, and so on.

- A **logical address** is a pair **(segment number `s`, offset `d`)** rather than a single number.
- A **segment table** holds, for each segment, a **base** (its starting physical address) and a **limit** (its length).
- Translation: index the segment table with `s`; check `d < limit` (trap on violation); the physical address is `base + d`.
...
Logical address: (s, d)
  ...
  translate (2, 53):  53 < 1100 OK  ->  4300 + 53 = 4353
```

This is essentially **base + limit per segment** instead of one base + limit for the whole process, so it gives each logical unit its own protection and lets segments grow independently and be shared at segment granularity. Its weakness is the same as variable-partition contiguous allocation: because segments have **arbitrary sizes**, the free physical memory between them gets chopped into odd-sized gaps — segmentation suffers **external fragmentation**. Real x86 systems (§9.1) therefore combine segmentation *with* paging: segmentation maps `(s, d)` to a linear address, and paging then maps that linear address to physical frames, eliminating the external fragmentation.
...
Each line shows the chosen hole and the leftover that hole becomes (in parentheses). The trace below was generated by simulation.

```text
First-fit (use the first hole large enough):
  P1(115) -> 300  hole  (185 left)
  P2(500) -> 600  hole  (100 left)
  P3(358) -> 750  hole  (392 left)
  P4(200) -> 350  hole  (150 left)
  P5(375) -> 392  leftover hole  (17 left)
  => ALL 5 requests succeed.

Best-fit (use the smallest hole that fits):
  P1(115) -> 125  hole  (10 left)
  P2(500) -> 600  hole  (100 left)
  P3(358) -> 750  hole  (392 left)
  P4(200) -> 200  hole  (0 left, exact fit)
  P5(375) -> 392  leftover hole  (17 left)
  => ALL 5 requests succeed.

Worst-fit (use the largest hole):
  P1(115) -> 750  hole  (635 left)
  P2(500) -> 635  leftover hole  (135 left)
  P3(358) -> 600  hole  (242 left)
  P4(200) -> 350  hole  (150 left)
  P5(375) -> FAILS
            (remaining holes: 300, 242, 150, 200, 135, 125 — largest is 300 < 375)
```

The lesson is not the exact numbers — it is that **the allocation strategy meaningfully changes which requests succeed**. Worst-fit always carves into the *largest* available hole, so it **shreds big holes early**: after spending the 750KB and then the 600KB holes on small/medium requests, no hole large enough for the final 375KB request remains, and **P5 fails** even though plenty of total free memory is left (this is external fragmentation). First-fit and Best-fit both preserve a large enough hole and complete all five requests.
...
```text
Three-level (4KB pages): keep the two inner levels page-sized
  (10 bits = 1024 entries each), leaving the outer level
  52 - 10 - 10 = 32 bits:
    outer table = 2^32 entries x 4 B = 2^34 bytes = 16 GB  → still too large
Pushing further: ~6-7 levels are needed before the outer
  table shrinks to a manageable size → per-reference walk
  cost (one memory access per level) balloons.
```
...
- Page sizes: **4 KB** (and **4 MB** large pages via **PSE — Page Size Extension**).
