...
1. `struct context`가 피호출자 저장 레지스터만 저장하는 이유는 무엇인가? 문맥 교환 시 호출자 저장 레지스터는 어떻게 되는가?
   ...
   > **정답:** `swtch`는 C 함수 호출이므로 RISC-V ABI상 호출자 저장(caller-saved) 레지스터는 호출자가 이미 저장/처리해 놓은 상태다. 따라서 호출 건너뛰어야 하는 **피호출자 저장(callee-saved)** 레지스터(s0–s11, sp, ra)만 보존하면 되며, `struct context`는 그것만 담는다.

2. `allocproc()`가 `p->context.ra`를 무엇으로 설정하며, 그 이유는? 새 프로세스로의 첫 번째 `swtch`에서 무슨 일이 일어나는가?

   > **정답:** `allocproc()`은 `ra`를 **`forkret`** 주소로 설정한다. 스케줄러가 새 프로세스 커널 스레드로 처음 `swtch`할 때 `ret`(=`ra` 사용)이 `forkret`으로 점프하고, 이것이 초기 락을 해제하고 `usertrapret`을 통해 사용자 모드로의 복귀 경로를 준비한다.

3. 스케줄러를 추적할 때 `CPUS=1`을 사용하는 이유는 무엇인가? `CPUS=3`을 사용하면 무엇이 달라지는가?

   > **정답:** 단일 CPU에선 스케줄링 이벤트가 순차적으로 발생해 `printf` 추적이 명확하다. `CPUS=3`이면 여러 CPU가 동시에 `swtch`·상태 변경·`printf`를 수행해, 출력 순서가 인과관계를 반영하지 못해 해석이 어려워진다.

4. xv6 스케줄러가 라운드 로빈 동작을 만들어내는 원인은 무엇인가? 현재 구현에 존재하는 미묘한 편향은 무엇인가?

   > **정답:** 스케줄러가 **`proc[]` 배열을 선형으로 순회**하며 RUNNABLE 항목을 찾으므로 자연스럽게 프로세스를 순환한다. **편향**: 배열 앞쪽 프로세스가 먼저 발견되어 우선 스케줄되므로, 엄격한 공정 RR이 아니라 낮은 인덱스 슬롯을 우대하는 방식이다.

5. 실습 4에서 `echo`가 파이프에 쓰기 전에 `cat`이 수면 상태에 들어가는 이유는 무엇인가? `echo`가 먼저 실행되면 어떻게 되는가?

   > **정답:** `cat`의 첫 `read()`가 빈 파이프에서 돌려줄 데이터가 없어 `sleep(channel)`로 들어가 대기한다. `echo`가 먼저 실행되면 `cat`의 `read()` 호출 시점에 파이프 버퍼에 이미 데이터가 있어 즉시 반환하고 sleep-wake 경로가 실행되지 않는다.

6. `wakeup()`이 깨어난 프로세스로 즉시 전환하지 않는 이유를 설명하시오. 이것이 `pthread_cond_signal`과 어떻게 다른가?

   > **정답:** `wakeup()`은 대기자 상태만 SLEEPING → RUNNABLE로 바꾸고 큐에 두며, 현재 프로세스는 스케줄링 지점(타이머, `yield`)까지 계속 실행한다. 즉시 전환하면 호출자의 크리티컬 섹션을 깨트릴 수 있다. `pthread_cond_signal`도 본질적으로 같다 — 대기자에게 "언젠가 실행하라"는 신호를 보낼 뿐 실제 언제 실행되는지는 스케줄러가 결정한다.
