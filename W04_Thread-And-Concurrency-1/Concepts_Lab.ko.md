...
`arg` 매개변수를 `struct thread_arg *`로 캐스팅한다 — 스레드에 할당된 ID를 저장하는 간단한 구조체이다: `struct thread_arg { int id; };`. 이 구조체는 전체 소스 파일에 정의되어 있다.

> **왜 공유 변수 대신 `partial_sum[id]`를 사용하는가?**
> - 각 스레드가 **자신만의 인덱스** 에 기록 — 충돌 없음
> - 모든 스레드가 하나의 `total` 변수에 기록하면 **경쟁 조건(Race Condition)** 이 발생 (교재 6장에서 다룸). **경쟁 조건(Race Condition)** 은 동시에 실행되는 스레드들이 공유 데이터에 접근하는 예측 불가능한 타이밍에 따라 프로그램 결과가 달라지는 버그이다 — 6장에서 자세히 다룬다.
...
> **시험 팁:** 이 인자 전달 함정은 자주 출제되는 주제이다. `pthread_create`는 **비동기적(Asynchronous)** 이다 — 새 스레드가 반드시 다음 줄이 실행되기 전에 시작되는 것이 아니라, 이후 어느 시점에든 실행을 시작할 수 있다. 근본 원인은 새 스레드가 즉시 실행되지 않을 수 있다는 것이다. 스레드가 포인터를 역참조할 때 이미 가리키는 값이 바뀌어 있을 수 있으며, 이것은 메인 스레드의 반복문과 자식 스레드의 포인터 역참조 사이의 경쟁 조건이다.
...
1. for 반복문에서 `&i`를 `pthread_create`에 전달하면 왜 잘못된 동작이 발생하는가?
   ...
   > **정답:** 모든 스레드가 같은 메모리 주소 `&i`를 공유하고, 루프가 `pthread_create` 이후에도 계속 `i`를 변경한다. 스레드가 `*arg`를 읽을 때쯤이면 루프가 이미 진행된 상태일 수 있어, 여러 스레드가 같은(혹은 오래된) `i` 값을 보게 되는 **race condition**이 발생한다. 해결: 스레드마다 별도 복사본을 주는 것 — `int tid[N]; tid[i] = i; pthread_create(..., &tid[i]);`.

2. 왜 하나의 공유 `total` 변수 대신 `partial_sum[id]`를 사용하는가?

   > **정답:** 여러 스레드가 동시에 `total += x`를 수행하면 read-modify-write가 원자적이지 않아 **race condition**이 발생하고 업데이트가 유실된다. `partial_sum[id]`는 각 스레드가 자신만의 슬롯을 쓰므로 경합이 없고, 메인 스레드가 join 후 합산하면 안전하고 올바른 결과를 얻는다.

3. 실제 속도 향상이 이상적 속도 향상에 미치지 못하는 요인은 무엇인가?

   > **정답:** ① 직렬 부분(암달의 법칙); ② 스레드 생성/join 오버헤드; ③ 캐시 미스·false sharing; ④ 메모리 버스·공유 자원 경합; ⑤ 락/대기 시간; ⑥ OS 스케줄링 지연과 문맥 교환 오버헤드.

4. 메인 스레드가 모든 자식 스레드에 `pthread_join`을 호출하기 전에 종료하면 어떻게 되는가?

   > **정답:** 프로세스가 종료되면 **모든 스레드가 강제 종료**된다 — 진행 중이던 작업이 사라지고, 버퍼링된 I/O가 flush되지 않을 수 있고, 파일 핸들·네트워크 소켓 같은 자원이 급하게 닫힌다. 결과를 신뢰할 수 없다. `main()` 반환 전에 항상 `pthread_join`(또는 의도적으로 detach)해야 한다.
