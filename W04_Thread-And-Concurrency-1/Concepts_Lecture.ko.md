...
1. 같은 프로세스의 스레드들이 공유하는 것은 무엇이고, 각 스레드가 독립적으로 소유하는 것은 무엇인가?
   ...
   > **정답:** **공유**: 코드 영역, 데이터 영역, 힙, 열린 파일 디스크립터, 시그널 핸들러, 프로세스 ID. **스레드별(독립)**: 스레드 ID, 프로그램 카운터, 레지스터 집합, 스택.

2. 같은 프로세스 내의 스레드 전환에서 TLB 플러시(flush)가 필요 없는 이유는 무엇인가?

   > **정답:** 같은 프로세스의 스레드는 **동일 주소 공간**(동일 페이지 테이블)을 공유하므로 가상→물리 매핑이 바뀌지 않는다. 기존 TLB 항목이 유효하므로 플러시할 필요가 없다. 반면 프로세스 간 문맥 교환은 주소 공간이 바뀌어 TLB 플러시 또는 ASID 태깅이 필요하다.

3. 프로그램의 직렬 코드 비율이 20%일 때, 암달의 법칙에 따라 8코어에서의 최대 속도 향상은 얼마인가? 무한 코어에서는?

   > **정답:** `Speedup = 1 / (S + (1-S)/N)`. S = 0.2, N = 8: `1 / (0.2 + 0.8/8) = 1/0.3 ≈ 3.33배`. N → ∞: `1 / 0.2 = 5배`. 아무리 코어를 늘려도 직렬 부분이 속도 향상을 **5배**로 제한한다.

4. 동시성(Concurrency)과 병렬성(Parallelism)의 차이를 예시와 함께 설명하라.

   > **정답:** **동시성**은 여러 작업을 논리적으로 겹치게 진행하는 것(단일 코어에서 시분할로 번갈아 실행 포함). **병렬성**은 여러 작업을 물리적으로 동시에 실행하는 것(다중 코어 필수). 예: 단일 코어에서 브라우저가 음악 재생과 페이지 로딩을 번갈아 = 동시성만. 쿼드코어에서 4개 스레드가 진짜 동시에 계산 = 병렬성.

5. 일대일(One-to-One) 멀티스레딩 모델이 사용자 스레드마다 커널 스레드를 생성하는 오버헤드가 있음에도 현대 운영체제에서 가장 널리 사용되는 이유는 무엇인가?

   > **정답:** ① 한 스레드가 블로킹 시스템 콜에 들어가도 다른 스레드는 커널에서 계속 실행 가능(Many-to-One의 치명적 한계 해결). ② 멀티코어에서 여러 스레드가 진짜 병렬 실행 가능. ③ 커널 스케줄러가 각 스레드의 우선순위를 개별 조정 가능. 스레드당 오버헤드는 커널 스레드 자료구조를 경량화해 상쇄되었고, 이점이 비용을 압도한다.
