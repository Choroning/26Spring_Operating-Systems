...
커널(Kernel)은 컴퓨터가 부팅될 때 시작되어 절대 종료되지 않는 핵심 프로그램으로, 모든 하드웨어 자원을 관리하고 응용 프로그램에 서비스를 제공한다.
...
> **정의:** **레지스터(Register)** 는 CPU 내부에 직접 내장된 초고속 소형 저장 공간으로, CPU가 현재 작업 중인 값을 보관한다. **상태 레지스터(Status Register)** 는 CPU의 현재 상태(예: 어떤 모드인지)를 나타내는 플래그를 저장하는 특수 레지스터이다.
...
특권 명령어(Privileged Instruction)는 잘못 사용하면 시스템에 해를 끼칠 수 있는 CPU 명령어로, 하드웨어가 커널 모드 외부에서의 실행을 물리적으로 차단한다.
...
- `syscall` 명령어는 (C 함수가 아닌) 특수한 CPU 수준의 명령으로, 하드웨어에 사용자 코드 실행을 중단하고 즉시 OS 커널로 제어를 넘기라고 지시한다.
...
> **참고:** 시스템 콜은 **번호(system call number)** 로 식별된다. 사용자 프로그램이 시스템 콜을 호출하면, 해당 시스템 콜 번호가 지정된 레지스터(RISC-V에서는 `a7`, x86에서는 `eax`)에 저장된 뒤 트랩이 발생한다. (`a7`과 `eax`는 특정 CPU 레지스터의 이름이다 — 하드웨어가 시스템 콜 번호를 확인하기 위해 검사하는 지정된 칸이라고 생각하면 된다) 커널은 이 번호를 읽어 **시스템 콜 테이블(syscall table)** 에서 대응하는 핸들러 함수를 찾아 실행한다. 3주차 실습에서 xv6의 시스템 콜 테이블(`syscall.c`)을 직접 확인하게 된다.
...
ISR(인터럽트 서비스 루틴)은 특정 인터럽트에 대응하여 실행되는 커널 함수이다. IVT(인터럽트 벡터 테이블)는 각 인터럽트 번호를 해당 ISR에 매핑하는 조회 테이블이다.
...
> **[컴퓨터구조]** 위로 갈수록 빠르고, 비싸고, 용량이 작다. 이 계층 구조가 작동하는 이유는 **지역성(locality)** 원리 때문이다. **시간적 지역성**: 최근 접근한 데이터는 곧 다시 접근될 가능성이 높다. **공간적 지역성**: 접근한 데이터 근처의 데이터가 곧 접근될 가능성이 높다. 책을 읽는 것에 비유하면, 50페이지를 읽은 후 다음으로 51페이지를 읽을 가능성이 높고(공간적 지역성), 곧 50페이지를 다시 읽을 수도 있다(시간적 지역성). 캐시 메모리는 이러한 예측 가능성을 활용한다. OS는 주기억장치 이하의 저장 장치를 관리하며, 가상 메모리(11~12주차)에서 이 계층 구조를 적극 활용한다.
...
> - **RISC-V 크로스 컴파일러**: `riscv64-unknown-elf-gcc` (또는 `riscv64-linux-gnu-gcc`) — x86/ARM 호스트에서 RISC-V 바이너리를 생성하기 위해 필요하다. 크로스 컴파일러가 필요한 이유는 보통 노트북이 x86(Intel/AMD)을 사용하지만 xv6는 RISC-V를 대상으로 하기 때문이다 — 크로스 컴파일러는 C 소스 코드를 노트북이 기본적으로 실행할 수 없는 RISC-V 기계어로 번역한다.
...
- `exec()`는 프로세스 이미지를 새로운 프로그램으로 **대체** 한다. 프로세스 이미지(Process Image)란 메모리에 있는 실행 중인 프로세스의 전체 내용 — 코드, 데이터, 스택, 상태를 말한다. `exec()`는 이 모든 것을 버리고 완전히 새로운 프로그램을 그 자리에 로드한다.
...
- xv6에서: RISC-V **Sv39** (Sv39는 RISC-V의 가상 메모리 방식에 붙인 이름이다 — 자세한 내용은 11~12주차에서 다루며, 지금은 가상 주소가 39비트 너비라는 것만 알아두면 된다) — 3단계 페이지 테이블, 39비트 가상 주소
...
1. 커널(Kernel)이란 무엇이며, 시스템 프로그램 및 응용 프로그램과 어떻게 다른가?
   ...
   > **정답:** **커널** 은 OS의 핵심으로 부팅부터 종료까지 메모리에 상주하며 하드웨어 자원 할당과 시스템 콜 처리를 담당한다. **시스템 프로그램** (셸, 컴파일러, 유틸리티)은 OS와 함께 배포되지만 사용자 모드에서 실행되며 사용자 환경을 제공한다. **응용 프로그램** 은 사용자가 설치해 실행하는 일반 소프트웨어. 커널만이 커널 모드에서 실행된다.

2. CPU에 두 가지 모드(사용자 모드와 커널 모드)가 필요한 이유는 무엇인가? 모드가 하나뿐이라면 어떤 문제가 발생하겠는가?

   > **정답:** 이중 모드는 사용자 프로세스가 특권 명령(I/O, 메모리 매핑 변경, halt 등)에 접근하지 못하게 격리해, 버그성·악의성 프로그램이 커널 자료구조나 다른 프로세스를 손상시키지 못하게 한다. 모드가 하나뿐이라면 어떤 프로그램이든 다른 프로세스 메모리를 덮어쓰거나 장치를 직접 제어하거나 인터럽트를 비활성화할 수 있어 한 프로그램 오류로 시스템 전체가 망가진다.

3. 사용자 프로그램이 `open()`을 호출할 때 발생하는 일련의 과정을 사용자 프로그램에서 커널까지, 그리고 다시 돌아오는 순서로 설명하라.

   > **정답:** ① 사용자 프로그램이 libc의 `open()` 래퍼를 호출 → ② 래퍼가 시스템 콜 번호와 인자를 레지스터에 설정하고 트랩 명령(예: RISC-V `ecall`) 실행 → ③ CPU가 커널 모드로 전환해 트랩 핸들러 진입 → ④ `sys_open()`이 파일 디스크립터 할당 후 inode 탐색 등 수행 → ⑤ 결과를 레지스터에 담고 `sret`로 사용자 모드 복귀 → ⑥ 래퍼가 반환 값을 사용자 프로그램에 전달.

4. 트랩(Trap)과 하드웨어 인터럽트(Hardware Interrupt)의 차이점은 무엇인가?

   > **정답:** **트랩** 은 소프트웨어가 발생시키는 **동기적(synchronous)** 이벤트로 시스템 콜이나 예외(0으로 나누기, 페이지 폴트)이며 항상 특정 명령어에 묶여 있다. **하드웨어 인터럽트** 는 외부 장치(타이머, 디스크, 키보드)가 보내는 **비동기적(asynchronous)** 신호로, 현재 실행 중인 명령과 무관한 시점에 도착한다.

5. 가상 메모리가 없다면 여러 프로그램을 동시에 실행할 때 어떤 문제가 발생하겠는가?

   > **정답:** ① 두 프로그램이 같은 물리 주소를 사용하면 서로의 메모리를 덮어쓴다. ② 물리 메모리 크기보다 큰 프로그램 실행 불가. ③ 격리가 없어 한 프로그램이 다른 프로그램이나 커널 메모리를 읽거나 수정 가능. ④ 재배치(relocation)가 불가능하므로 메모리 단편화로 빈 공간을 제대로 활용하지 못한다.
