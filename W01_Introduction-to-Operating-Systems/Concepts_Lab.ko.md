...
> **선수 지식**: 1주차 이론 개념 (OS 정의, 이중 모드, 시스템 콜).
>
...
> **참고:** 일반 응용 프로그램은 OS가 메모리 배치를 관리해 주지만, OS 자체를 개발할 때는 "코드가 메모리 어디에 올라갈지"를 직접 지정해야 한다. **링커 스크립트(linker script)** 가 바로 이 역할을 수행한다. 예를 들어, 부트로더는 반드시 메모리 주소 `0x7C00`에 위치해야 하고, 커널 코드는 그 뒤에 배치되어야 하는데, 이를 링커 스크립트에서 `.text`, `.data`, `.bss` 섹션의 시작 주소와 순서로 지정한다. `.text`는 실행 가능한 코드를, `.data`는 초기화된 변수를, `.bss`는 초기화되지 않은 변수를 저장한다. `0x7C00`은 BIOS가 부트 섹터를 배치하는 메모리 주소로, 수십 년 전에 확립된 하드웨어 규약이다. 링커 스크립트가 없으면 컴파일러가 임의의 주소를 사용하여 부팅에 실패한다.
...
1. 코딩 에이전트와 Copilot 같은 단순 코드 자동완성 도구의 핵심 차이점은 무엇인가?
   ...
   > **정답:** Copilot은 커서 위치의 코드 조각 하나를 자동완성한다. **코딩 에이전트**는 파일 읽기/쓰기·빌드·테스트·명령 실행 등 여러 단계 도구를 호출하며 목표에 맞춰 자율적으로 계획·실행·검증한다. 에이전트는 한 줄이 아닌 프로젝트 전체 상태 위에서 작동한다.

2. 파일 시스템 작업을 실행하기 전에 에이전트에게 "계획을 먼저 보여줘"라고 요청하는 것이 왜 중요한가?

   > **정답:** 파일 시스템 작업은 삭제·덮어쓰기처럼 비가역적일 수 있다. 에이전트가 의도를 잘못 해석하면 복구가 어렵다. 계획을 먼저 검토하면 범위 오류(잘못된 경로, 의도치 않은 삭제)를 디스크 변경 전에 발견하고 수정할 수 있다.

3. RALPH 기법을 자신의 말로 설명하라. 루브릭을 활용한 반복 개선이 단일 프롬프트보다 더 나은 결과를 만드는 이유는 무엇인가?

   > **정답:** RALPH(Rubric-Aligned Loop Prompt Hardening)는 명확한 평가 기준(루브릭)을 정의한 뒤, 에이전트가 결과물을 그 기준에 비추어 자가 평가·개선을 반복하게 한다. 단일 프롬프트는 한 번의 추측에 의존하지만, 루브릭 기반 루프는 각 수정이 측정 가능한 기준으로 향하게 해 품질이 꾸준히 수렴한다.

4. 미니 OS를 구축할 때 부트로더(Bootloader)의 역할은 무엇이며, 리얼 모드(Real Mode)에서 프로텍티드 모드(Protected Mode)로 전환해야 하는 이유는 무엇인가?

   > **정답:** 부트로더는 펌웨어가 가장 먼저 실행하는 작은 프로그램으로, 커널을 메모리에 로드하고 제어권을 넘긴다. x86 리얼 모드는 16-bit이며 1 MB 주소 공간만 접근 가능하다. 현대 커널은 32/64-bit 주소와 메모리 보호가 필수이므로, 부트로더가 커널 점프 전 반드시 프로텍티드 모드로 전환해야 한다.

5. 에이전트가 생성한 OS 코드가 빌드에 실패했을 때, 에이전트에게 수정을 요청하는 가장 효과적인 방법은 무엇인가?

   > **정답:** 전체 에러 메시지, 실패한 명령, 환경(OS·컴파일러 버전)을 함께 전달하고 명확한 성공 조건("이 에러를 고치고 기존 테스트도 통과")을 제시한다. 컨텍스트가 구체적일수록 에이전트가 원인을 추측할 여지가 줄어든다.
